function getLastLoseGame(callBack, allGameCount) {
  ...
  database.onsuccess = function (event) {
    ...
    transaction.addEventListener("complete", function (e) {
      console.log(
        "Streak用トランザクションが終了。Transaction for Streak done"
      );
...
function getStreak(prevLostTime, allGameCount) {
  ...
  database.onsuccess = function (event) {
    ...
    console.log(
      `The ID of the game you lost is "${prevLostTime}", which means DB doesn't have any data and this is your first game.`
    );
    let db = event.target.result;
    let transaction = db.transaction(storeName, "readonly");
